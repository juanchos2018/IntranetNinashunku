...
export default {
  ...
  mounted() {
     // this.getTipoUsers();
      if (this.datos) {   
       this.InfoNoticiaEvento(this.datos);
    }
  },
  methods: {
    Validate,
    ...
    InfoNoticiaEvento(id){
      let me = this;
      var items = [];     
      let url =me.url_base+ "Control/EventoNoticiaEspacio.php?id_noticiaeventoespacio="+id;
      axios({
        method: "GET",
        url: url,
      })
      .then(function(response) {
          //console.log(response)
           me.modelNoticiaOrga = response.data.result;     
           me.fileList.push({
                uid: '-1',
                name: 'image.png',
                status: 'done',
                url: me.url_base +me.modelNoticiaOrga.photo,
                },)    
            })
        .catch((error) => {
          console.log(error);
    ...
    limpiarcampos(){
      ...
      this.modelNoticiaOrga.linkvideo = ""
      this.fileList=[];
    }
